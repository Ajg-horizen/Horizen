...
function MetricCard({
  ...
  return (
    <div className="rounded-2xl border border-foreground/[0.08] bg-background p-6">
      ...
      {hint && (
        <p className={`mt-2 text-xs ${statusColor}`}>{hint}</p>
      )}
    </div>
  );
}
...
function PriorityBadge({ priority }: { priority: 1 | 2 | 3 | 4 }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-[10px] font-medium uppercase tracking-wider ${color}`}>
      {text}
    </span>
  );
}

function SeoStatusBadge({ status }: { status: "done" | "in_progress" | "pending" | "not_applicable" }) {
  const cfg = {
    done: { icon: CheckCircle2Icon, text: "SEO færdig", color: "text-emerald-600 bg-emerald-50 border-emerald-200" },
    in_progress: { icon: ClockIcon, text: "I gang", color: "text-amber-600 bg-amber-50 border-amber-200" },
    pending: { icon: CircleDashedIcon, text: "Venter", color: "text-foreground/50 bg-foreground/[0.04] border-foreground/[0.08]" },
    not_applicable: { icon: MinusIcon, text: "Ikke relevant", color: "text-foreground/30 bg-foreground/[0.02] border-foreground/[0.06]" },
  }[status];
  const Icon = cfg.icon;
  return (
    ...
    <main className="min-h-screen bg-background py-12 md:py-16">
      <Container size="site">
        ...
        <div className="mt-16">
          <SectionHeader
            title="Action items"
            description="Ting du selv skal gøre når du har tid"
          />
        ...
        <div className="mt-16">
          <SectionHeader
            title="Keywords vi tracker"
            description="Aktuelle positioner i Google"
          />
          <div className="overflow-hidden rounded-2xl border border-foreground/[0.08] bg-background">
            <table className="w-full">
              <thead className="border-b border-foreground/[0.08] bg-foreground/[0.02]">
                <tr>
              ...
              <tbody className="divide-y divide-foreground/[0.06]">
                {keywordsTracked.map((kw) => (
                  <tr key={kw.page}>
                    <td className="px-5 py-4 text-sm font-medium">{kw.page}</td>
                    <td className="px-5 py-4 text-sm">{kw.primary}</td>
                    <td className="px-5 py-4 text-right text-sm tabular-nums">
                      {kw.currentPosition}
                    </td>
                    <td className="px-5 py-4 text-right text-sm tabular-nums">
                      {kw.impressionsLast28d}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="mt-16">
          <SectionHeader
            title="Sider, status"
            description="Eksisterende sider og deres SEO-status"
          />
          <div className="overflow-hidden rounded-2xl border border-foreground/[0.08] bg-background">
            <table className="w-full">
              <thead className="border-b border-foreground/[0.08] bg-foreground/[0.02]">
              ...
              <tbody className="divide-y divide-foreground/[0.06]">
                {existingPages.map((p) => (
                  <tr key={p.url}>
                    ...
                    <td className="px-5 py-4 text-sm text-foreground/60 font-mono text-xs">
                      {p.url}
                    </td>
                    <td className="px-5 py-4">
                      <SeoStatusBadge status={p.seoStatus} />
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="mt-16">
          <SectionHeader
            ...
            description="Prioriteret efter SEO-impact (AI-sporet først)"
          />
          <div className="grid gap-3 md:grid-cols-2">
            {upcomingPages.map((p) => (
              <div
              ...
              >
                ...
                <div className="flex shrink-0 items-center gap-3 text-xs text-foreground/50">
                  <span>Impact: {p.impact === "high" ? "🟢" : p.impact === "medium" ? "🟡" : "⚪"}</span>
                  <span>Konk.: {p.competition === "low" ? "🟢" : p.competition === "medium" ? "🟡" : "🔴"}</span>
                </div>
              </div>
            ))}
          </div>
